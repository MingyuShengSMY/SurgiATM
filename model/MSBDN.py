...
dense feature fusion. In Proceedings of the IEEE/CVF conference on computer vision and pattern recognition (pp. 2157-2167).

This code is built from GitHub:
    ...
    def forward(self, ft_h, ft_l_list):
        ...
        if self.mode == 'iter2':
            ...
            for i in range(len(ft_l_list)):
                ...
                if ft.size()[2:] != ft_l_list[i].size()[2:]:
                    ft = F.interpolate(ft, ft_l_list[i].size()[2:], mode="bilinear")
                ft = ft - ft_l_list[i]
                for j in range(self.num_ft - i):
                    ft = self.up_convs[self.num_ft - i - j - 1](ft)
                if ft.size()[2:] != ft_fusion.size()[2:]:
                    ft = F.interpolate(ft, ft_fusion.size()[2:], mode="bilinear")
    ...
    def forward(self, ft_l, ft_h_list):
        ...
        if self.mode == 'iter2':
            ...
            for i in range(len(ft_h_list)):
                ...
                ft_i = ft_h_list[i]
                if ft.size()[2:] != ft_i.size()[2:]:
                    ft = F.interpolate(ft, ft_i.size()[2:], mode="bilinear")
                ft = ft - ft_h_list[i]
                for j in range(self.num_ft - i):
                    # print(j)
...
class MSBDN(BaseModel):
    def __init__(self, res_blocks=18):
        super().__init__()

        self.net = Net(res_blocks)
    ...
    def get_loss(self, de_smoke_x, gt):
        loss = F.mse_loss(de_smoke_x, gt)
    ...
    def get_loss_dict(self, batch_dict, only_dict=False):
        ...
        loss = self.get_loss(de_smoke_x=de_smoke_x, gt=gt)
